...
public class FileManage {
    ...
    public void showProductSelect(File file, String selectType) throws FileNotFoundException {
        ...
        double discount = 0;
        if (checkRole().equals("2")){
            discount = 0.05;
        }
        if (checkRole().equals("3")){
            discount = 0.10;
        }

        System.out.println("============ " + category.get(selectType) + " ============");
        String title = discount != 0 ? String.format("%-5s %-17s %-10s %12s \n", "#" ,"Name", "Price (฿)", "Quantity") : String.format("%-5s %-15s %-10s %6s \n", "#" ,"Name", "Price (฿)", "Quantity");
        System.out.print(title);
        for (Product i : productList){
            if (i.getId().equals(checkTypeId)){
                double price = (Double.parseDouble(i.getPrice().substring(1)) * 34);
                double priceDiscount = discount != 0 ? price - (price * discount) : price;
                String productData = discount != 0
                        ? String.format("%-5s %-15s %.2f (%.2f) %8s \n", countProduct, i.getName(), priceDiscount, price, i.getQuantity())
    ...
    public void sortMaxMin(){
        ...
        for (int i = 0; i < productList.size()-1; i++) {
            ...
            temp = productList.get(i);
            for (int j = i+1; j < productList.size(); j++) {
                if (Double.parseDouble(productList.get(min).getPrice().substring(1)) < Double.parseDouble(productList.get(j).getPrice().substring(1))){
                    min = j;
                }
            }
            productList.set(i, productList.get(min));
